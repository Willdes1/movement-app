...
const READINESS_DATA = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Coach voice cloning — clients hear cues in their coach’s voice (premium)', done: true },
...
function OverviewSection() {
  const BUILT = [
    'Coach signups stay coaches. Choosing "I\'m a coach" at signup was quietly failing: the app remembered that choice in the browser tab it was made in, but the normal path is to sign up, open the confirmation link from your email, and land in a brand new tab, where that memory is gone. So new coaches were being created as ordinary athletes, dropped into the athlete app, and then asked the athlete fitness questionnaire, which is skipped for coaches and was never meant to reach them. Google and Apple signups usually returned to the same tab, which is why this survived unnoticed while the most common path stayed broken. The choice now travels on the account itself, so it survives a new tab, a different browser, or a different device entirely, and it is applied once and then cleared so a deliberate role change later is never undone. Found while onboarding our first real coach. Files: app/auth, app/auth/callback, contexts/AuthContext.tsx. No SQL.',
    'The setup questionnaire now sticks, and can be skipped on purpose. Two faults. First, a failed save was completely invisible: the code was watching for a kind of error the database library never raises, so answers could be typed into every screen, saved to nowhere, and the flow would carry on as if all was well. On the next login the app saw an empty profile and correctly concluded the athlete had never onboarded, so it asked everything again. If a save fails now it says so on screen, in plain words, and it refuses to move to the next step rather than throwing the answers away. Second, "already done" was only remembered in the browser, so a second device, a private window or clearing site data meant starting from scratch. That is now recorded properly against the account. Skipping is also a real, remembered answer rather than an absence, which means someone who skips is not ambushed on every single login, and can be reminded later and fill it in from their Profile whenever they want. Questions no longer arrive unexplained either: sport, goals and training location each carry a "Why we ask" note saying exactly what the AI does with the answer, and the skip button, which did exist but was faint grey text in a corner nobody found, now reads "Skip all of this". Sports and goals also both accept more than one answer, since a lifter who snowboards and skates was previously forced to pick one. Files: components/OnboardingModal.tsx, lib/sports.ts. SQL: 20260807_onboarding_status.',
    'Coaches get asked whether they train themselves. A coaching account has always included the full athlete app, but nothing ever asked the coach about their own training, so the AI had nothing to work from and could only write them something generic. The coach setup now asks once, directly, with a plain explanation that this is not a fitness quiz and they are the professional: it is simply the only thing the AI needs to write them a real program. Say yes and they pick their own sports; say no and that is recorded so they are never asked again. Both ways of training yourself stay open and are spelled out: let the AI build it from the sports picked, or build a program by hand in the builder and assign it to yourself, which shows up in your own app under My Coach. Files: components/coach/OnboardingOverlay.tsx, lib/sports.ts.',
    'Partner setup in one command, with proof. Giving a teammate their role and their admin sections meant working across two different admin screens, and a role change against a missing profile row silently does nothing, which looks identical to having worked. There is now a single command that sets one person\'s role, grants exactly the sections named, then reads it all back and prints who on the platform has any admin access at all, so the list can be eyeballed. It is deliberately hard to misuse: it refuses anything that is not one email address, refuses if two accounts share that address, refuses to touch an owner or admin, and refuses outright to hand out anything sensitive (users, billing, spend, CEO briefing) or the Access Control master key, which have to stay a considered decision in the Access Control screen. Access is granted per person, never to a role, so granting one coach can never grant every coach. Files: scripts/provision-partner.js.',
    'Every new coach starts with a real library, not an empty screen. 52 standard movements (squats, deadlifts, bench, rows, presses, dips, carries and the plain ab work) are now waiting the first time a coach opens their Exercise Library, with the coaching instructions already written. It costs nothing to hand over: those exercises already exist in the 2,050-row library with cues and narration you have paid for, so this is a copy and no AI is involved. Videos are left blank on purpose so coaches attach their own YouTube links or upload their own footage, and never have to clear out choices we made for them. Everything is an editable example: rewrite the cues, rename it, or delete it. Sets, reps, rest and how much weight moved out of the library and into the program builder, where they belong, because the same Bench Press is 3x12 for one client and 5x5 at their 12-rep max for another. Also removed a trap: a button labelled "Set rest" was actually converting the whole day into a rest day and deleting every exercise on it. Files: lib/coach-starter-library.ts, app/api/coach/seed-library, components/coach/ManualProgramBuilder.tsx. No SQL.',
    'The app now handles a program running out, instead of pretending it did not. This was a launch blocker: once an athlete had used up their 13 weeks, Start Session (the main button on Home) led to a completely blank page. Home and the Calendar had drifted into two different ideas of what day it was. Finishing your program now gives you a proper moment, and two free ways to carry on: pick up where you left off, which slides the week you stopped on onto today and keeps your progress, or restart the same plan from day one. Neither costs a generation, because they only move the dates on the plan you already own. Previously the only way forward was rebuilding all 13 weeks from scratch, so anyone who took a month off had to pay to regenerate a plan they already had. Home also notices when you have drifted a couple of weeks behind and offers to line the dates back up, and it never moves anything without you choosing it. A timezone bug went with it that was making Home and the Calendar show different workouts for the same day. Files: lib/program-progress.ts, app/api/user/program-restart, components/ProgramResumeActions.tsx. No SQL.',
    'One definition of an exercise, everywhere. The same movement used to look different depending on which page you opened it from. The plan page had its own hand-built copy of the exercise popup with different headings, no video and no read-aloud, and it never even asked the database for those things, so they could not have appeared. The browse page was loading narration audio on every visit and showing no play button. The anatomy page quietly dropped the core engagement cue. Two pages were running every query twice, with a reduced retry that hid real errors behind what looked like a missing video. All of it now reads one shared definition, the plan page uses the real popup (so video, read-aloud, per-section playback and set history all arrive at once), and browse, exercises and mobility finally have the read-aloud button they already had the audio for. Nothing about the design changed: the same pages simply show what we already had. Files: lib/exercise-columns.ts, components/ui/ReadAloudButton.tsx, docs/library-render-audit.md. No SQL.',
    'Unilateral video cleanup. Exercises that train one side at a time (single-arm rows, single-leg balance, alternating curls) were carrying videos approved before the matcher ever checked for that, so some of them demonstrate both arms or both legs. The Library Cleanup tab now sweeps the entire library and gives you the real number: 29 of them carry an approved video, and 28 have never been trimmed, meaning nobody has watched those frame by frame. It sorts them by how risky each one is, since a trimmed clip had a human pick the exact rep and is far likelier to be right. Tick the ones you want and confirm, and they drop back into the video curation backlog to be matched again. Counting is read only and nothing is ever reset in bulk. This also caught a live naming bug: a broken rule had been leaving a full stop stranded mid-name ("Alternating. Dumbbell Curls"), which the narration was reading out as a sentence break. Rule fixed, and a repair pass added so the rows already renamed that way get corrected too. Files: app/api/admin/unilateral-audit, components/admin/UnilateralAuditPanel.tsx, lib/exercise-naming.ts. No SQL.',
    'What happens when a program runs out. This was a launch blocker hiding in plain sight: once an athlete\'s 13 weeks elapsed, Start Session (the main button on Home) led to a completely blank page. Home and the Calendar had drifted into two different ideas of what day it was. Home asked which week you were in and capped the answer at 13, so it cheerfully replayed the final week forever; the Calendar worked from real dates and correctly had nothing, so it drew an empty screen. Now both read from one place, an athlete who finishes gets a proper "your 13 weeks are complete" moment with a route into a fresh block, and the Calendar can never render nothing. Two related faults went with it: the Calendar always opened on the month the program started (April for an athlete in August), and the focused day view stuck to the Calendar tab so that tab was blank too. The Program Complete screen already existed with the right buttons and was unreachable, because it was gated on the current week having no plan, which meant anyone who generated all 13 weeks up front (the recommended path) never saw it. Files: lib/program-progress.ts, app/today, app/calendar, app/plan. No SQL.',
    'Read-aloud you can actually control. The narration used to be all or nothing: it read the whole instruction set start to finish, and tapping the speaker again restarted it rather than stopping it. Now every instruction block has its own speaker button, so an athlete mid-set can hear just the breathing cue without sitting through the rest. While anything is playing, a control bar appears at the bottom of the screen with pause, resume, stop, a draggable progress bar, a speed control (0.75x up to 1.5x), and the male/female voice switch, which previously was buried inside one popup where most athletes never found it. The section being read is highlighted as it plays. Speed and voice are remembered. Three real bugs went with it: two clips could play over the top of each other, audio could start playing after the athlete had already navigated away, and the app was re-billing OpenAI to regenerate narration it had already generated and saved. Speed cost nothing to add and works on all the existing audio, so none of it had to be regenerated. Files: contexts/TTSContext.tsx, components/ui/TTSMiniPlayer.tsx, lib/speech-text.ts. No SQL.',
    'YouTube curation now costs almost nothing to run — the single biggest cost fix so far. Curating one exercise used to burn 201 of the 10,000 daily YouTube units, and hitting Regenerate burned about 1,500 because it silently searched every approved channel. That capped the whole library at roughly 19 videos a day. Now the app keeps its own local index of every video your approved channels have ever posted (18,300 of them, cached once for 372 units) and matches exercises against it in your own database, for free. A paid YouTube search only happens when the local index genuinely has nothing, and it is capped per day, counted, and labelled in the run log so a paid call can never happen invisibly. Regenerate is now free. A live quota meter shows units used, units left, and hours until the midnight reset. Channels can be added by handle for 1 unit each (the old auto-discover cost 500 a run and was also producing broken channel IDs — six of nine were dead, including every rehab and mobility source, now repaired). Files: lib/youtube.ts, lib/video-matching.ts, lib/exercise-abbreviations.ts, lib/movement-classification.ts, app/api/admin/youtube-*. SQL: 20260725_youtube_uploads_index, 20260729_match_channel_videos.',
    'Dead video detection at zero API cost, plus no more broken players. If a creator deletes a video or turns off embedding, the athlete used to see a blank black box and you would only find out from a complaint. Now the video player itself reports the failure the moment it happens, you get an alert in the Telemetry tab naming the exercise, and the athlete sees a clean "video coming soon" card instead of a broken box. It costs nothing to run: no scheduled scanning, no API quota, and it only ever flags videos someone actually tried to watch. Files: components/ui/LoopPreview.tsx, lib/youtube-iframe.ts, app/api/video-health/report.',
    'Video Trimming tab — the montage queue. The trimmer already worked, but there was no way to see how many videos still needed trimming or to work through them without hunting one at a time. Now there is a progress bar and counters (total, trimmed, remaining, needs review), filters, and a Next-untrimmed button that jumps you straight to the next one. Each row opens the trimmer inline, and if the video is simply wrong you can paste a replacement link right there (which clears the old trim, since those timings belonged to the old video). Every save records who trimmed it and when, so a teammate can be given this one tab through Access Control and work the queue for you. The trimmer itself was also rebuilt: you can now click or drag the scrub bar to jump anywhere in the video, with 5s/1s/0.2s nudge buttons, instead of having to sit and watch until the right moment arrived. SQL: 20260731b_trim_status.',
    'Mobility movements now show video. The mobility library was written instructions only. It turned out the videos were never really missing: mobility movements are ordinary library rows that could already be curated and trimmed, but the mobility page never asked the database for the video columns. Now every mobility movement shows the same short looping montage the workout view uses, with a tap through to the full video and a VIDEO badge so you can see at a glance which are done. Files: app/mobility/page.tsx.',
    'Library Cleanup tab — fix abbreviated exercise names safely. Names like "1 DB Chest Fly" were breaking video matching and, worse, being read aloud by the narration exactly as written. This tab proposes proper names ("Single-Arm Dumbbell Chest Fly"), keeps whatever equipment the name states and never invents equipment it cannot see, and leaves already-correct names alone. Crucially it shows the consequences before you approve anything: whether that exercise has a video, whether its narration will go stale, and whether a movement that trains one side at a time is carrying a video that shows both. Approving a rename automatically clears that exercise\'s audio so it regenerates saying the right name, and the original name is kept invisibly for tracing. Nothing is ever applied in bulk without approval. SQL: 20260731_exercise_legacy_name.',
    'Search that forgives how people actually type. Searching "cat cow" or "catcow" used to return nothing, because the movement is stored as "Cat-Cow Stretch" and nobody types the hyphen. The same flaw was quietly hiding movements from their own body-area filters. Search now ignores punctuation, handles plurals ("hips", "calves", "stretches"), matches words in any order, and offers "did you mean" suggestions for typos like "pigon" or "couch strech". Applied to Mobility, Exercises, and Browse, and verified against 33 realistic misspellings. Files: lib/fuzzy-search.ts.',
    'Content auto-pilot — the blog now runs itself. The engine auto-drafts articles on a schedule to keep a buffer waiting for you, you approve the good ones with one click into a drip queue, and approved articles publish automatically to atlasprime.app/blog on your chosen days (default Tuesday and Friday). A daily background job handles it all. You control everything from the Auto-pilot panel in the Marketing tab: toggle auto-generation, pick publish days, set how many drafts to keep buffered, and reorder or unqueue what is lined up. Nothing ever goes live without your approval, so quality stays high and there is no thin-content risk. This turns content marketing into a thumbs-up while you build the app. Files: app/api/cron/content, app/api/admin/content/settings, lib/content-generate.ts, components/admin/MarketingTab.tsx, vercel.json. SQL: 20260721_content_scheduling. Needs CRON_SECRET in Vercel (added this session).',
    'Content Engine — the Marketing Hub goes live (Phase 1). The admin Marketing tab is now a real content engine: pick a topic cluster (coach software, recovery, AI training, sport-specific), give a topic, and the Atlas Prime engine drafts an expert, human-voiced SEO article grounded in the domain knowledge store. You review and edit it, then publish to a public blog at atlasprime.app/blog. Each article ships with its own SEO title, meta description, Article structured data, and gets added to the sitemap automatically, so it can rank in Google. Strategy: this wins the search terms the "Atlas Prime" video-game brand collision cannot touch (AI workout plan, coaching software, etc.) — inbound traffic that funnels into signups. Human-in-the-loop by design (no thin AI spam). Also previews the rest of the Marketing Hub (lead investigation, AI outreach, ads). Files: components/admin/MarketingTab.tsx, app/blog, app/api/admin/content, lib/{content,content-clusters,markdown}.ts. SQL: 20260720_content_posts.',
    'SEO launch — the marketing site is now findable. Homepage got a keyword-worked title and description, a code-generated branded share card (the orange-on-black "Train like you mean it." image that unfurls when the link is texted or posted), a sitemap and robots file, and an FAQ section with rich-result structured data. Verified live in Google Search Console (auto-verified via the business domain) with the sitemap submitted, and imported into Bing. Files: app/layout.tsx, app/opengraph-image.tsx, app/twitter-image.tsx, app/sitemap.ts, app/robots.ts, components/landing/LandingPage.tsx.',
    'Public marketing landing page — atlasprime.app now has a real homepage. Logged-out visitors land on a full marketing site (the "Kinetic" design); logged-in users still go straight to the app. Built from the actual app palette (near-black + orange-red #FF5C35 + emerald, with a blue trust accent for the coach side) so the website and the logged-in product finally feel like one brand, grounded in real cited color and bounce-rate research (dark base + one accent, single-stat hero, high-contrast CTA, no heavy video). Splits cleanly into For Athletes and For Coaches, each with product mockups and its own pricing on a toggle, plus an animated EKG pulse hero, SEO hooks (structured data), and a login link. Fully responsive down to a small phone. Reversible via LANDING_LIVE in lib/flags.ts, and the logo is a single swappable placeholder. Also added a self-serve "I am an athlete / I am a coach" toggle to signup so coaches self-identify without a special link. Files: components/landing/LandingPage.tsx, app/page.tsx, app/auth/page.tsx.',
    'Coach billing — Stage A foundation (not live yet). A /coach/billing page shows the coach their plan and live monthly usage (AI programs + messages) with tier cards that upgrade through the existing Stripe checkout, plus a Manage-billing portal link. Pricing lives in one editable file (Free, Starter 19, Growth 49, Pro 99; credits + message caps). Usage is tracked in a coach_usage table. A platform seam (isNativeApp) keeps web on Stripe now and leaves room for in-app purchases on iOS later. Everything is gated behind BILLING_LIVE so it turns on in one flip once the Stripe products are created. Model decided with the owner: monthly credits with a hard cap and upgrade prompt (automatic overage billing is a later phase). Files: lib/coach-plans.ts, lib/coach-usage.ts, app/coach/billing, app/api/coach/billing-status.',
    'Product Telemetry — a Product view in the admin Telemetry tab that tracks what customers actually click through. Every page view is recorded, plus named conversion events at the moments that matter (signup, coach signup, generate-plan, plan generated, coach AI program used, program created, client invited, program activated, workout complete). The Product view shows total events, unique users, page views today, top pages, a dedicated conversion-events breakdown, a 7-day activity chart, and a live recent-activity stream. Separate data model (product_events) from the harness system-health events. This is the funnel Will will use to see where athletes and coaches convert or drop before wiring paid billing. Files: lib/track.ts, components/ProductTracker.tsx, app/api/track, app/api/admin/product-events, components/admin/ProductTelemetry.tsx.',
    'Harness Engineering Stages 2-4 — the monitoring, tracking, and testing layers on top of Stage 1. STAGE 2 (monitoring): a harness_events table + a new Admin → Dev Tools → 📡 Telemetry tab surfacing every time the app\'s own safety checks fire (severity badges, newest-first), a DSN-gated Sentry forwarder + a withHarness() route wrapper that reports thrown route errors to both Sentry and the tab (no-ops cleanly until a SENTRY_DSN is added — like the voice-cloning flag), and a 🔥 Fire-test-error button that proves the whole capture path. STAGE 3 (migration safety — the one Will personally wanted): an applied_migrations ledger where every migration self-registers when you run it, a live 🗄 Migrations panel that tells you if you ever forgot to run one (it immediately caught 2 unrun migrations on day one → now 33/33), a migration-drift CLI, and staging/dry-run docs. STAGE 4 (smoke tests): a server-side /api/admin/smoke-test suite that exercises the critical paths and asserts real DB side effects — including a token_usage verified write→read-back→cleanup that actively tests the exact money-logging bug class that started this whole effort — with a ▶ Run smoke tests button + persistent last-run status, and a post-deploy CLI. A smoke run caught its own false-positive (gate check hitting a Vercel-protected URL) and it was fixed same session. Files: lib/observability.ts, components/admin/TelemetryTab.tsx, app/api/admin/{harness-events,migration-drift,smoke-test}, scripts/{migration-drift,smoke-test,migrations-manifest}.js, docs/harness-*.md. SQL: 20260707_harness_events, 20260708_applied_migrations (both run → the app now knows, and tells you, when a safety check trips or a migration is missing).',
    'Harness Engineering Stage 1 — read-after-write verification on money/state writes. New lib/verified-write.ts (verifiedInsert/Update/Upsert: write → read the row back → assert key fields persisted; on failure logs a loud [VERIFY_FAIL] + a harness_events row and throws) + lib/harness-events.ts (safe before the Stage-2 table exists). Retrofitted the writes that could silently fail: logTokens (loud-but-contained so cost logging can\'t 500 a request), coach assignment activate/pause status changes, and coach session logging (workout_logs + day completions). An admin-only /api/admin/harness-selftest + a "Run harness self-test" button on the Architecture tab prove the loud-failure path. This is the exact class of bug that hid the logTokens failure for months. Stages 2-4 (harness_events table + Sentry + Telemetry tab, staging DB, smoke tests) are next.',
    'System Architecture doc — end-to-end map of how data flows + how the backend works, three ways kept in sync: ARCHITECTURE.md (dev), docs/architecture.html (visual, shareable), and a native Admin → Dev Tools → 🗺️ Architecture tab. Covers the stack layers, the two-Supabase-client request pattern, roles/identity, the APIE plan pipeline, coached mode, the content engine, cost tracking, the DB by domain, external services, the security model, and deploy. Provider color-coded (Claude/OpenAI) to match the Spend Tracker.',
    'Coach signup link — atlasprime.app/coaches (QR-ready). Coach-branded signup that auto-assigns role=coach and drops them straight into the Coach Portal (works email + Google/Apple via a pendingCoach flag; admins protected by protect_admin_role). This is the Coach business-card QR target; the athlete QR is the root domain. Admin Promos dropdown also gains a "Coach" option for controlled invites. Decided pricing: 5 free AI programs, then monthly tiers with included AI credits + overage charges (billing not live yet). Files: app/coaches/page.tsx, app/auth + /auth/callback.',
    'Spend Tracker — receipts + tax package + edit + dates. Receipt uploads were the only client-side upload in the app (failed on storage RLS, error swallowed) with no `receipts` bucket — now a server-side /api/admin/upload-receipt (service role) + a receipts-bucket migration, and the upload REFUSES to save an expense whose receipt failed. Added: ✎ Edit on every expense (fix the date or attach a receipt without re-logging), 📦 Tax Package zip (JSZip: expenses.csv + every receipt file, cross-referenced via a "Receipt File" column — one file for a tax preparer), per-operation AI-cost date ranges, a timezone fix so date-only tax dates don\'t render a day early, and a ↻ Refresh button. Files: components/admin/SpendTab.tsx, app/api/admin/upload-receipt/route.ts.',
    'Library Builder — token-safety + bulk audio. (1) "🔊 Voice All Missing Audio" — a hands-free loop that voices the entire audio backlog (~1,900 exercises seeded before auto-audio or cleared by an Upgrade) 10 at a time until done, with progress + stop. Auto-audio only covers brand-new rows; this covers everything else. (2) Fill Every Category now has a saturation guard: once a category yields 0 new exercises at a given count it\'s remembered (localStorage) and skipped on future runs WITH ZERO token cost — so accidental re-runs on an already-full library cost nothing. Bumping the count higher re-probes; a "Reset & re-check all" link clears the memory. Fixes the real problem where re-running Fill silently re-charged Claude to regenerate duplicates. Files: components/admin/LibrarySeedTab.tsx, app/api/admin/generate-tts/route.ts.',
    'Spend Tracker now captures EVERY AI call — was silently broken. The shared logTokens() helper wrote with the public anon key (RLS-blocked) to a non-existent `route` column, so every server-side AI cost 400\'d and was swallowed — only the client-side exercise_backfill ever showed. Fixed to use the service-role key + real column (api_route). Then added full OpenAI cost tracking on top of Claude: TTS (tts-1, per-character), embeddings (text-embedding-3-small, per-token), and Whisper transcription (whisper-1, per-minute) — each logs its exact dollar cost via a new lib/ai-costs.ts pricing module, and the tracker sums the stored per-row cost so mixed providers (Claude tokens + OpenAI chars/tokens/minutes) roll up accurately. Every operation row now shows a Claude/OpenAI provider badge. So the total is finally the real number. Files: lib/log-tokens.ts, lib/ai-costs.ts, components/admin/SpendTab.tsx + the 4 OpenAI routes (generate-tts, tts, coach/transcribe, knowledge-retrieval).',
    'Desktop "fill the screen" UX pass — every athlete tab + the coach portal now fill the width on desktop/laptop (they were capped narrow and floating in the middle with big empty gutters). For You feed → 2-column grid; My Coach / Nutrition / Account / Profile widened to 1200; coach pages centered at 1280. The coached Calendar was rebuilt from an abstract week/day picker into a real month grid that matches the AI-plan calendar exactly (seamless when a client switches off a coach to the AI plan), and its mobile horizontal cutoff was fixed. Profile-name emoji now shows in EVERY greeting (account "Hey, X", Home, desktop sidebar, mobile menu) via a shared lib/name.ts helper that keeps the first name + emoji while avatar initials stay clean — plus a preloaded emoji picker on the profile name field (tap to add/remove flair). Mobile left untouched throughout (all width changes gated to >=768px).',
    'Library Builder (admin) — one-click bulk-preload of the exercise library across every sport + training focus. Pick any sport/category (or type your own) + a count, and Claude generates real, distinct exercises WITH full instructions (how / breathing / core / common-mistake tip) in a single batched call; it de-dupes against exercise_library (never regenerates what we already have, and remembers per-category so re-runs go deeper), inserts the new ones, and queues them for the Video Curator — TTS auto-picks them up because rows arrive with instructions. Feeds both content pipelines so the platform is preloaded (no lag when new users generate plans) and the YouTube curator always has a daily backlog. Cheap by design: Haiku default ≈ $0.001/exercise (Sonnet selectable), tokens logged to Health Monitor; live counters for library total / awaiting-video / awaiting-audio. NEW: 🔊 Auto-generate audio toggle (on by default) — every newly-seeded exercise is handed straight to the TTS route (OpenAI onyx + nova) as it\'s added, chunked to stay under the Vercel 60s wall, so a single "Fill Every Category" run now preloads text, video-queue, AND voice in one pass (no separate TTS run). Claude writes the words, OpenAI speaks them — two providers, one button. No migration — reuses exercise_library + exercise_video_candidates. Files: app/api/admin/seed-library/route.ts, components/admin/LibrarySeedTab.tsx, app/api/admin/generate-tts/route.ts.',
    'Coached Mode v2 — full coach-athlete program experience, on par with the AI plan. When an athlete is on a coach program it TAKES OVER (its own calendar via CoachedCalendar, workable days with video/cues/coach-voice/set-tracking; AI plan paused). Coach exercise INSTRUCTIONS: standard fields (How/Breathing/Core/Common-Mistakes) shown free from the global library, a full coach editor (structured + custom fields, Autofill-free + AI-generate, shared CoachInstructionFields component) reachable in the Library AND in-place on the program page (✎ Cues), plus "import program exercises to my Library", a per-coach Field Template (choose/reorder standard fields), and a one-click SELF-HIDING admin bulk-curation panel across all programs. PROGRAM SWITCHING: coach assign is now athlete-confirmed (pending → push + in-app Activate prompt, no silent takeover); save/resume/restart (resume at the week you left off vs start fresh Day 1, logs/PRs kept); "pause coaching → back to my own AI plan" with paused programs resumable from My Coach — one active program at a time, nothing lost. Fixed a launch-level bug where coached programs never displayed (my-program selected a non-existent coach_programs.description column). SQL: 20260630_coach_exercise_instructions/field_template/assignment_pending/assignment_resume_week.',
    'Coach voice cloning (Phase 1, premium) — "Your coach, in their ear, every rep." A coach records a short consent + voice sample on their Dashboard (record in-browser or upload) → ElevenLabs Instant Voice Cloning creates a custom voice_id stored on coach_voices. When a coached client taps 🔈 on an exercise, the cues are synthesized in their actual coach\'s voice and cached once per (coach, exercise) in a public bucket (coach_exercise_audio) so repeat taps are free — mirrors the exercise_library tts_url_* pattern. Falls back to the default narrator if the coach has no voice or anything fails. Gated by the COACH_VOICE_CLONING flag + the server ELEVENLABS_API_KEY (graceful "being set up" state without the key). Commercial use needs a paid ElevenLabs tier (natural coach-tier upsell). Files: lib/elevenlabs.ts, app/api/coach/voice + /voice/speak, components/coach/VoiceCloneCard.tsx. SQL: 20260623_coach_voice_cloning.sql',
    'Coach library reaches the client — closing the loop: when a client does a coach-built workout, each exercise now prefers the coach\'s OWN library clip + cues over the global database (★ "Coach\'s demo" badge, YouTube clip or uploaded video + the coach\'s instructions/notes). Resolved server-side in /api/coach/my-program (service role, since RLS blocks the client from reading the coach\'s library) and threaded through CoachedContext into CoachedSessionCard. Library → Builder → Client is now end-to-end.',
    'Coach Exercise Library → Program Builder connection — in the manual program builder, each day\'s exercise autocomplete now pulls the coach\'s OWN library first (tagged ★ LIBRARY, 🎥 video marker, their saved sets×reps), then the global database; selecting a library exercise autofills name + sets/reps. Added a "📚 From Library" button per day that opens a picker to browse + multi-select exercises straight from the coach\'s library. Makes building programs from a coach\'s curated, video-backed library fast. Files: components/coach/ManualProgramBuilder.tsx, components/coach/LibraryPickerModal.tsx. No schema change.',
