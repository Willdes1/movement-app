...
export default function HomePage() {
  ...
  const startProgram = useCallback(async () => {
    ...
    const { data: created } = await supabase
      ...
      .single()
    if (!created) { setDataLoading(false); return }
    const prog: Program = { id: created.id, startDate: created.start_date, totalWeeks: created.total_weeks, status: created.status }
    setHasProgram(true)
    setProgram(prog)
    setCurrentWeek(1)
    setViewingWeek(1)
  ...
  useEffect(() => {
    setLastLog(null)
    setLogWeight(''); setLogSets(''); setLogReps(''); setLogSaved(false)
  ...
  async function handleCompleteDay(weekNum: number, dayIdx: number) {
    ...
    if (weekNum === currentWeek) {
      ...
      if (missed.length > 0) {
        ...
        setPendingComplete({ weekNum, dayIdx })
  ...
  async function markAllAndComplete() {
    ...
    for (let i = 0; i <= pendingComplete.dayIdx; i++) {
      ...
      if (!completions.has(key) && plan[i].type !== 'rest') {
        inserts.push({ user_id: user.id, program_id: program.id, week_number: pendingComplete.weekNum, day_index: i, skipped: false })
      }
    }
    if (inserts.length > 0) {
  ...
  async function restructureAndComplete() {
    ...
    const missed = missedDays.join(' and ')
    setRegenInstructions(`I missed ${missed}. Please restructure the remaining days this week to make up for the missed sessions without overloading any single day.`)
    setPendingComplete(null)
  ...
  async function restartProgram() {
    ...
    setProgram(null); setWeekPlans({}); setCurrentWeek(1); setViewingWeek(1); setProgramComplete(false); setHasProgram(false)
    initProgram()
  }
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <div style={{ padding: '24px 16px 16px' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>{getDayLabel()}, {getDateLabel()}</div>
        ...
        <div style={{ margin: '0 16px', background: 'linear-gradient(135deg, var(--accent-bg) 0%, rgba(0,0,0,0) 100%)', border: '1px solid var(--accent-border)', borderRadius: 18, padding: '20px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 20, background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', fontSize: 10, fontWeight: 800, color: 'var(--accent)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 10 }}>● Recovery Active</div>
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ margin: '0 16px', background: 'linear-gradient(135deg, var(--accent-bg) 0%, rgba(0,0,0,0) 100%)', border: '1px solid var(--accent-border)', borderRadius: 18, padding: '20px' }}>
          ...
          <button onClick={startProgram} style={{ display: 'block', width: '100%', padding: '14px', borderRadius: 12, background: 'linear-gradient(135deg, var(--accent) 0%, var(--accent2) 100%)', color: '#fff', fontWeight: 900, fontSize: 14, textAlign: 'center', border: 'none', cursor: 'pointer', letterSpacing: '0.03em', textTransform: 'uppercase', boxShadow: '0 6px 24px var(--accent-shadow)' }}>
            Get Started →
          </button>
        </div>
      </div>
  ...
  if (programComplete && !weekPlans[viewingWeek]) {
    return (
      <div style={{ padding: '60px 24px', maxWidth: 480, margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-dim)', fontSize: 14, marginBottom: 32, lineHeight: 1.6 }}>You finished all 13 weeks. That&apos;s a serious achievement — your fitness has transformed. What&apos;s next?</p>
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', paddingBottom: 100 }}>
      ...
      <div style={{ padding: '24px 16px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>{getDayLabel()}, {getDateLabel()}</div>
          <h1 style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.15 }}>
            {getGreeting()}{firstName ? ', ' : '. '}<span style={{ color: 'var(--accent)' }}>{firstName ?? 'Athlete'}.</span>
          </h1>
        </div>
        <button
          onClick={() => setShowRegenModal(true)}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', background: 'none', border: '1px solid var(--accent-border)', borderRadius: 20, padding: '6px 12px', cursor: 'pointer', marginTop: 4, flexShrink: 0 }}
        >
      ...
      <div style={{ display: 'flex', gap: 8, padding: '0 16px 12px' }}>
        <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '10px 8px', textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: phaseColor }}>{currentWeek}<span style={{ fontSize: 11 }}>/13</span></div>
          <div style={{ fontSize: 9, color: 'var(--text-dim)', fontWeight: 700, marginTop: 2, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Week</div>
        </div>
        <div style={{ flex: 2, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '10px 8px', textAlign: 'center' }}>
          <div style={{ fontSize: 13, fontWeight: 900, color: getPhaseInfo(currentWeek).color, paddingTop: 2 }}>{getPhaseInfo(currentWeek).label.replace(' Phase', '')}</div>
          <div style={{ fontSize: 9, color: 'var(--text-dim)', fontWeight: 700, marginTop: 2, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Phase</div>
        </div>
        <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '10px 8px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, fontWeight: 900, color: 'var(--text-dim)' }}>—</div>
          <div style={{ fontSize: 9, color: 'var(--text-dim)', fontWeight: 700, marginTop: 2, letterSpacing: '0.06em', textTransform: 'uppercase' }}>No Injury</div>
        </div>
      </div>

      {/* Week navigation */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 16px 8px', padding: '10px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12 }}>
      ...
      <div style={{ margin: '0 16px 14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          <span style={{ fontSize: 11, color: phaseColor, fontWeight: 700 }}>{phaseLabel}</span>
      ...
      {error && (
        <div style={{ padding: '12px 16px', background: 'var(--orange-bg)', border: '1px solid var(--orange-border)', borderRadius: 10, margin: '0 16px 14px', fontSize: 13, color: 'var(--text)' }}>{error}</div>
      )}

      {/* Day strip */}
      <div style={{ display: 'flex', gap: 8, margin: '0 16px 14px', overflowX: 'auto', paddingBottom: 2 }}>
        {DAYS.map((d, i) => (
          <div key={d} style={{ flexShrink: 0, width: 40, height: 40, borderRadius: 10, background: i === todayIdx && viewingWeek === currentWeek ? 'var(--accent)' : 'var(--surface)', border: `1px solid ${i === todayIdx && viewingWeek === currentWeek ? 'var(--accent)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          return (
            <div key={day.day} style={{ background: isToday ? (TYPE_BG[day.type] ?? 'var(--surface)') : 'var(--surface)', border: `1px solid ${isToday ? (TYPE_BORDER[day.type] ?? 'var(--border)') : 'var(--border)'}`, borderRadius: 12, padding: '14px 16px' }}>
              ...
              {actionable && !isRest && !completed && (
                <button
                  onClick={() => handleCompleteDay(viewingWeek, i)}
                  disabled={completing}
      ...
      {viewingWeek === TOTAL_WEEKS && plan.length > 0 && (
        <div style={{ margin: '16px 16px 0', padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, textAlign: 'center' }}>
      ...
      {/* Tip of the day */}
      <div style={{ margin: '16px 16px 0', padding: '14px 16px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, borderLeft: '3px solid var(--accent)' }}>
        ...
        <div onClick={() => setShowCompleteModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 18, padding: 24, width: '100%', maxWidth: 420, border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button onClick={markAllAndComplete} disabled={completing} style={{ padding: '13px', borderRadius: 10, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer', textAlign: 'left' }}>
              ...
              <button onClick={restructureAndComplete} disabled={completing} style={{ padding: '13px', borderRadius: 10, border: '1px solid var(--accent-border)', background: 'var(--accent-bg)', color: 'var(--accent)', fontWeight: 700, fontSize: 14, cursor: 'pointer', textAlign: 'left' }}>
                ⚡ Restructure My Week
                <div style={{ fontSize: 11, fontWeight: 500, opacity: 0.8, marginTop: 3 }}>AI rebuilds your remaining days to make up for missed sessions</div>
              </button>
              <button onClick={async () => { if (pendingComplete) { await doMarkComplete(pendingComplete.weekNum, pendingComplete.dayIdx); setPendingComplete(null) } setShowCompleteModal(false) }} disabled={completing} style={{ padding: '13px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                Just Mark Today
              </button>
            </div>
          </div>
        ...
        <div onClick={() => { setShowRegenModal(false); setRegenInstructions('') }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 18, padding: 24, width: '100%', maxWidth: 420, border: '1px solid var(--border)' }}>
            ...
            <textarea
              ...
              placeholder={'e.g. "No workouts on Saturday" or "I missed Mon and Tue — rebuild the rest of the week"'}
              rows={3}
              style={{ width: '100%', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: 13, padding: '10px 12px', resize: 'none', fontFamily: 'inherit', marginBottom: 18, boxSizing: 'border-box', lineHeight: 1.5 }}
            />
            <div style={{ display: 'flex', gap: 10 }}>
