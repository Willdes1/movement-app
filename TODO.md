...
- [ ] **7. Per-subsection audio controls on instructions** — On workout + calendar pages, the Breathing / Core engagement / Common mistakes sections currently force listening to the whole audio explanation. Add: play the entire section with one click AND play each subsection separately, so users can jump to just what they need. Touches the TTS playback layer (`useTTS`, ExerciseDetailModal, workout blocks).
