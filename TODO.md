...
- [ ] **🔥 PRIORITY — Coach logs workout on client's behalf (in-person sessions)** — For coaches who train clients in person: a "Log Session" flow in the coach portal (client detail page) where the coach picks the program day (defaults to today's scheduled day) and fills out, per exercise: sets completed, reps completed, weight, plus session-level coach notes and any adjustments made during the session. Writes `workout_logs` rows for the client (needs a `logged_by` column so entries show "Logged by Coach X") + `coach_day_completions` so the day shows ✓ completed on the client's Today card, and stats/history/PBs all reflect it. Service-key API route with roster verification (coach can only log for own active clients).
