...
- [ ] **2. "Massage Gun Tips" educational tab** — New content tab on percussion-massage usage (generic language; avoid Theragun/trademark issues). Cover: where to use, when to use, areas to avoid, recovery recommendations. Same shape as the existing "Got Extra Time for Abs?" / "Cool Down" educational blocks. Ties into recovery content.

- [ ] **3. Claude as Will's sales coach (ongoing behavior, not a feature)** — After building any feature, Claude proactively teaches Will how to SELL it: benefits over features, no feature-dumping, speak like a founder / think like a salesperson, explain why it matters to the end user. Also retroactively reposition older features. Example framing: "This matters because when a coach builds a program, Atlas Prime will eventually flag a workout that may be inappropriate given the athlete's history, injuries, or recovery." Consider capturing this as a standing session habit (memory/feedback). See [[ceo-education-agent]].

- [ ] **4. Slide-by-slide athlete onboarding (replace the scrolling flow)** — One-question-per-slide onboarding (name → sport → experience → style → equipment → gym/home/both). Any free-text answer gets the AI voice input already built in the Coach Portal (MediaRecorder + Whisper), with copy: "Just speak into your microphone. We'll auto-correct punctuation. Press Improve Grammar to clean it up, or type if you prefer." Must be skippable at any point (straight into the app), with later gentle nudges ("Would you like to finish your profile?"). Expand over time. See [[onboarding-help-knowledge-base]], [[profile-improvements]].

- [ ] **5. AI profile optimizer (anytime)** — Let a user update/optimize their profile whenever they want by speaking OR typing to the AI. The AI updates the profile and auto-regenerates future recommendations + training plans using our internal knowledge library. Pairs with #4's voice input. See [[profile-improvements]].

- [~] **6. Marketing → full Lead Investigation Hub (Admin Portal)** — **PHASE 1 SHIPPED 2026-07-20: the Content Engine.** The Marketing tab is now live: APIE-grounded SEO article generation (4 topic clusters) → review/edit → publish to public /blog with per-article metadata + Article schema + sitemap. Files: components/admin/MarketingTab.tsx, app/blog, app/api/admin/content, lib/{content,content-clusters,markdown}.ts. SQL: 20260720_content_posts (RUN REQUIRED). Remaining phases below still to build: search businesses by industry, local or worldwide (PT clinics, PT studios, commercial gyms, enterprise chains like 24 Hour Fitness / LA Fitness); classify Independent / Multi-location / Enterprise and score each lead; find owner / decision-maker / email / phone; export lists. Plus: AI outreach templates, personalized email generation, a smarter Mailchimp-style sender that reduces Gmail/Outlook deliverability issues, and Google / Facebook / Instagram / TikTok ads management. Dedicated AI agents for lead gen, prospect research, email/DM/SMS drafting, cold-call scripts, keyword recs, audience targeting, campaign suggestions. Also: find personal trainers on social, generate personalized DMs, suggest outreach per coach profile. Even if outreach is sent manually, AI prepares everything first. **Big multi-phase build.** See [[partner-outreach-agent]], [[retention-marketing]]. **← This is where tonight's SEO work becomes the top of the funnel (see session notes).**

- [x] **7. Per-subsection audio controls on instructions** — ✅ Built 2026-08-04 as curation-queue Task 5. Each instruction block in the exercise modal has its own speaker button, alongside the existing play-all. Also shipped: a mini player (pause / resume / stop / seek / speed / voice) that appears on every surface while audio plays, section highlighting during play-all, and speed + voice preferences persisted. `contexts/TTSContext.tsx` replaces the per-component `useTTS` hook so one audio element serves the whole app. Fixed with it: the toggle restarted instead of stopping, two clips could overlap, audio could start after navigating away, and `/api/tts` re-billed OpenAI for audio that already existed. No SQL.

- [ ] **8. Wearable device integration** — Connect Atlas Prime to Oura, Apple Watch, Samsung Galaxy Watch, Garmin, Fitbit, and other major trackers. Especially valuable in the Coach Portal: even when an athlete forgets to log sets/reps/notes, Atlas Prime detects the activity from the wearable so the coach sees "Athlete completed a workout today, but nothing was logged" and can follow up ("Did you finish today's session? I saw activity but no log. Adjust tomorrow?"). Long-term: makes coaches proactive and accountable with every client. Large integration (per-vendor OAuth + health APIs).

## 🆕 Added 2026-06-11

- [x] **🔥 PRIORITY — Coach logs workout on client's behalf (in-person sessions)** — ✅ Built 2026-06-11. Log Session section on client detail page: week/day/date pickers, per-exercise sets/reps/weight prefilled from programmed scheme, per-exercise notes + session notes (→ coach_client_notes), client's own logs preload with "edit or leave as is" badge, Mark day complete toggle. Writes workout_logs (logged_by=coach) + coach_day_completions (completed_by=coach). Client gets push notification + one-time "Good job! 🎉" banner; day shows "Completed with Coach X". SQL: 20260611_coach_session_logging.sql.
- [ ] **"No sport? Tell us what you do for work"** — Profile option for non-athletes: occupation-based plan generation. Desk worker, construction, barber, nurse, driver — AI builds a plan around the physical demands and problem patterns of their job (posture, repetitive strain, mobility deficits) so they move better and feel great. Entry point: sport selector gets a "No sport? Click here" path that asks about their work.
- [ ] **Program-style preference in profile + PDF style transfer** — New profile step: "Is there a program style you like?" with examples (ATHLEAN-X, Knees Over Toes Guy, Bamberger-style mobility, etc.). If "Yes, I follow a different program" → ask which one(s); user can mix influences ("Muscle Beach method strength + KOT knee work + Bamberger mobility"). Bottom of step: "Have a PDF workout from your favorite program?" → upload; intelligent text extraction that ignores background images/symbols/branding (vision fallback already exists in import pipeline). Output rebuilt in OUR house style: workouts, sets, reps, rest times, breathing techniques, core engagement, common mistakes, instructions — APIE generates with the style influence baked in.
- [ ] **Founder's Program** — Will personally tests his own generated plan starting now through Kallen's wedding (~mid-July 2026), intense version, firsthand product QA. After the wedding: review how it felt, improve + intensify. Long-term: list it in Featured Programs as "Try our Founder's Program" once the brand is established.
- [ ] **Coach calculators toolkit (brainstorm + research)** — Quick-tool calculators in the coach portal: 1RM estimate (Epley/Brzycki), percentage-of-max loading (e.g., total reps at 10–12 then 50%/75% of that), training volume (sets × reps × load), RPE→%1RM conversion, plate math, rest timer. Research what tools coaches actually use today (TrueCoach, TrainHeroic, spreadsheet workflows), identify what's missing, keep ours unique — possibly context-aware (pulls the client's logged PBs straight into the calculator).
...
- [x] **Workout flow UX pass (built 2026-06-17)** — dashboard heads-up card (video removed); Start Session → focused single-day view (`/calendar?focus=1`, month grid hidden, "View Calendar" one tap away); read-before-you-start overview at the TOP (assembled instructions + equipment + rest-by-type) + durable "Week X · Day Y" header; progressive disclosure (workout = hero; morning/abs/cooldown/evening = friendly collapsed invitation cards revealed one-at-a-time on scroll via IntersectionObserver sentinel); coached card gets the overview + optional per-day coach walkthrough video (ManualProgramBuilder field → day JSON, no migration). No SQL. Docs: `docs/APP_GUIDE.md`.
- [x] **Workout UX restructure + set tracking (built 2026-06-17)** — dashboard summary (time/equipment/preview/For-You); calendar day view with inline looping previews + Coaching Cues per exercise, Day-N header, equipment + rest-by-type box; reusable ExerciseDetailModal (Common Mistakes = `tip`) with per-set Track Workout (reps/weight/notes, duplicate/add set, opt-in L/R) + per-exercise History tab; dismissible tracking tips. SQL: `20260617_exercise_set_logs.sql`. Shared via /calendar so coached clients get it too. Docs: `docs/APP_GUIDE.md`.

## 🗂️ Admin Tools (added 2026-04-23)
- [x] **Admin partner permissions + portal stays private at store submission (built 2026-06-18)** — Admin portal must NOT be exposed in the Apple App Store / Google Play submission — it's for the owner (Will) + future partners only (gated, never shown to normal users/reviewers). Owner vs partner model: owner = profiles.is_owner (sees all + the owner-only 🔑 Access Control tab); partners = admin_permissions table (NOT profiles.is_admin, so they can't read owner-only data and revocation isn't blocked by the protect_admin_role trigger). Access Control menu: pick a user, check sections to grant, edit/suspend/revoke anytime. Private (never grantable): overview, users, zoom log, retention, billing, stripe, spend, ceo, notes, bugs, conversions, todos, ideas, promos, health, access. Grantable: launchpad, study, kb, mie, media, video, tts, marketing, partners, push. Server enforces per-tab via verifyAdmin(req, tab)/verifyOwner. Admin link added to both navs for owner/partners (hidden during impersonation). SQL: 20260618_admin_permissions.sql. Files: lib/admin-tabs.ts, lib/admin-auth.ts, app/api/admin/access, components/admin/AccessControlTab.tsx, contexts/AuthContext.tsx.
- [x] **Admin Study Hub (built 2026-06-18)** — Admin-only certification study hub (ISSA CFT depth) in the admin portal (Strategy group → 🎓 Study Hub). Multi-KB from the start: each Knowledge Base is a subject container that keeps its own context + scoped system prompt layered on the APIE engine. One-tap generation of original structured study material (concepts / summaries / key points / quizzes / flashcards) grounded in our pgvector domain store via retrieveKnowledge(); save/tag/search, mastered/active status, and progress tracking toward the cert goal. Copyright guardrail baked into the prompt (original content only — no ISSA/Hatfield/NASM/ACE reproduction). Server-only access (JWT → service-role → is_admin gate, RLS deny-by-default). Files: components/admin/StudyHubTab.tsx, app/api/admin/study/{kbs,entries,generate}, lib/admin-auth.ts. SQL: supabase/migrations/20260618_study_hub.sql (study_kbs + study_entries).
...
- [x] **In-app account deletion + Contact Support (built 2026-06-18)** — Apple Guideline 5.1.1 requires in-app account deletion; it was missing AND the legal docs falsely claimed it existed. Built a "Delete My Account" funnel on /account (intent: take-a-break vs delete → "why are you leaving?" reason capture → tailored save offer [take a break / contact support / discount placeholder gated by BILLING_LIVE flag] → type-DELETE confirm). Server route /api/account/delete derives the user from their JWT (self-only), captures an anonymous reason to account_deletion_feedback, best-effort wipes their data across tables, then auth.admin.deleteUser. Also added a "Contact Support" form (ContactSupportModal) writing to the existing bug_reports pipeline — no email exposed (use a free support@atlasprime.app Workspace alias for the store console contact field). Privacy policy updated to disclose OpenAI. SQL: 20260618_account_deletion.sql. Files: components/account/DeleteAccountModal.tsx + ContactSupportModal.tsx, app/api/account/delete/route.ts.
